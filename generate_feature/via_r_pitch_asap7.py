...
def get_via_pitch_csv(x_pth,x):
    ...
    df.to_csv(f"asap7_ncsv/pitch{x}.csv", index=False, header=None)
    save_to_csv(data_array_viar,
                f'asap7_ncsv/via{x}.csv')
    np_data_array_layer_r = np.array(data_array_layer_r, dtype=np.float32)
    save_to_csv(np_data_array_layer_r, f'asap7_ncsv/r{x}.csv')

for x in range(1000):
    x_pth='/Users/xxx/asap7'
