...
def read_fake_data_test_sky130hd(folder_path,add_pth):
    ...
    with open(file,'w') as f:
        ...
        for i in index_list:
              ...
              if os.path.exists(tcsv):
                   ...
                   # Ap = to_pil_image(tensor1)
                   # Bp = to_pil_image(tensor2)
                   ...
                   ssimx = ssim(matrix2, matrix1, multichannel=True)
                   # print("ssim", ssimx)
...
def test_one(net,device,xdata,i,norm):
        ...
        if norm==True:
            ...
            xdata = xdata.unsqueeze(dim=0)
            pred = net(xdata)
        print(i,xdata.shape,pred.shape)
...
pth='/home/wangmingyue/date/sky130hd'
add_pth = '/home/wangmingyue/date/sky130hd_ncsv'
...
model=MSU_attNet(img_ch=6,output_ch=1,xmax=xmax,xmin=xmin,ymax=ymax,ymin=ymin)
model.load_state_dict(model_state_dict)
dirpth = f'results/{args.tdir}'
os.chdir(dirpth)
# read testing data
test_x,test_y,index_list= read_fake_data_test_sky130hd(pth,add_pth)
test_y=count(test_x,test_y)
print(len(test_y))
# test loop
for i,x,y in zip(index_list,test_x,test_y):
    x=x.squeeze()
    test_one(model, device, x, i,norm=True)
# evaluate
re_real(test_y,index_list)
