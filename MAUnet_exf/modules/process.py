...
def read_fake_data_test_sky130hd(folder_path):
    ...
    # tfile_pattern = 'BeGAN_{:03d}_voltage_map_regular.csv'
    add_pth='/home/wangmingyue/sky130hd_netcsv'
    ...
    return data_list, tdata_list,index_list
#
def read_fake_data_nangate45(folder_path,add_pth):
    ...
    for i in range(1000):
        ...
        if (os.path.exists(tfile_path)):
            ...
            texpand_torch = torch.unsqueeze(texpand_torch, 0)
            tdata_list.append(texpand_torch)
            shapex.append(int(texpand_torch.shape[-1]))
            print(int(texpand_torch.shape[-1]))

    print(max(shapex),min(shapex))


    return data_list,tdata_list

def read_fake_data_nangate45_test(folder_path,add_pth):
    file_pattern=['BeGAN_{:04d}_current.csv','BeGAN_{:04d}_eff_dist.csv','BeGAN_{:04d}_pdn_density.csv']
    tfile_pattern='BeGAN_{:04d}_ir_drop.csv'
        ...
        if(os.path.exists(tfile_path)):
            ...
            for pattern in file_pattern:
                ...
                if concat_data is None:
                    ...
                    concat_data=torch.cat((concat_data,expand_torch),dim=0)
            # for matrix in matrix_patterns:
            #     matrix_pth='/home/wangmingyue/fake_case'
            #     file_pth=os.path.join(matrix_pth,matrix.format(i))
            #     csv_content=pd.read_csv(file_pth,header=None)
            #     torch_content=torch.tensor(csv_content.values)
            #     # torch_content=torch.flip(torch_content,[0])
            #     expand_torch=torch.unsqueeze(torch_content,dim=0)
...
def read_real_data(folder_path):
    data_list=[]
    tdata_list=[]
    train_set = [1,2,3,4,5,6,11,12,17,18]
    # matrix_pth = "/home/wangmingyue/real_case"
    # vias_pth="/home/wangmingyue/real_case"
    ...
    for i in train_set:
        ...
        if os.path.exists(file_path):
            ...
            #     expand_torch = torch.unsqueeze(torch_content, dim=0)
            #     concat_data = torch.cat((concat_data, expand_torch), dim=0)
            # for via in vias_patterns:
            #     file_pth = os.path.join(vias_pth, via.format(i))
            #     csv_content = pd.read_csv(file_pth, header=None)
            #     torch_content = torch.tensor(csv_content.values)
            #     # torch_content=torch.flip(torch_content,[0])
            #     expand_torch = torch.unsqueeze(torch_content, dim=0)
            #     concat_data = torch.cat((concat_data, expand_torch), dim=0)
            # # for via in r_patterns:
            #     file_pth = os.path.join(r_pth, via.format(i))
            #     csv_content = pd.read_csv(file_pth, header=None)
            #     torch_content = torch.tensor(csv_content.values)
            #     # torch_content=torch.flip(torch_content,[0])
            #     expand_torch = torch.unsqueeze(torch_content, dim=0)
...
def read_all_real_data(folder_path):
        data_list = []
        tdata_list = []
        train_set = [1,2,3, 4,5,6, 11, 12, 17, 18]
        matrix_pth = "/home/wangmingyue/real_case"
...
# tpath='/home/wangmingyue/Desktop/iccad/fake-circuit-data/'
# data_list,tdata_list=read_fake_data(tpath)
# wx,wtx,hx,htx,whx,whtx=flip(data_list,tdata_list)
# data_list=data_list+wx
# data_list=data_list+hx
# data_list=data_list+whx
...
# hlist=[]
# for cadc1063_alpha in data_list:
#     hlist.append(cadc1063_alpha.shape[0])
# heights,bins,_=plt.hist(hlist,bins=50)
# for i in range(len(heights)):
#     plt.annotate(str(int(heights[i])),xy=(bins[i]+0.5,heights[i]),ha='center',va='bottom')
# plt.xlabel('value')
# plt.ylabel('count')
# plt.show()
